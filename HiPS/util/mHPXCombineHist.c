...
{
   int    c, ntile, order, width, offset, gap, nhist, nfail;
   ...
   while ((c = getopt(argc, argv, "do:w:")) != EOF) 
   {
      switch (c) 
      {
         ...
         default:
            printf("[struct stat=\"ERROR\", msg=\"Usage: mHPXCombineHist [-d] [-o order] [-w width] roughdir smoothdir\"]\n");
            exit(1);
            break;
      }
   }
   ...
   for(jtile=0; jtile<ntile; ++jtile)
   {
      for(itile=0; itile<ntile; ++itile)
      {
         ...
         for(j=jtile-offset; j<=jtile+offset; ++j)
         {
            for(i=itile-offset; i<=itile+offset; ++i)
            {
               ...
               if(gap == NONE)
               {
                  ...
                  sprintf(tilename, "%sNpix%d.hist", roughdir, id);
               }
         ...
         sprintf(cmd, "mCombineHist -stretch min max gaussian-log -out %s %s", outname, tilelist);
   ...
   printf("[struct stat=\"OK\", module=\"mHPXCombineHist\", nhist=%d, nfail=%d]\n",
          nhist, nfail);
   fflush(stdout);
   exit(0);
}
