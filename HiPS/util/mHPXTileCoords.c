...
void mHPXTileCoords_transform (int level, int tile, int *x, int *y);
void mHPXTileCoords_splitIndex(long index, int level, int *x, int *y);
...
/*  coordinates, then pixel coords and finally (using the HPX WCS) sky   */
/*  coordinates.                                                         */
/*                                                                       */
/*************************************************************************/
...
int main(int argc, char **argv)
{
   ...
   if(argc < 3)
   {
      printf("[struct stat=\"ERROR\", msg=\"Usage: mHPXTileCoords [-d] level tileID\"]\n");
      exit(1);
   }
   ...
   mHPXTileCoords_transform(tileLevel, tileID, &xtile, &ytile);
...
void mHPXTileCoords_transform(int level, int tile, int *xtile, int *ytile)
{
   int  baseTile, nside;
   long index;
   int  i, x, y;
   ...
   mHPXTileCoords_splitIndex(index, level, &x, &y);
