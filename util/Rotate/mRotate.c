...
int main(int argc, char **argv)
{
   ...
   {
      input.crota2 = atan2(-input.cd12, input.cd22)/dtr;

      if(debug)
      {
         printf("crota2 (from cd12/cd22) = %-g\n", input.crota2);
         fflush(stdout);
      }
      ...
      if(debug)
      {
         printf("crota2 (from cd21/cd11) = %-g\n\n", input.crota2);
         fflush(stdout);
      }
   ...
   if(haveRegion)
   {
      ...
      maxoff = sqrt(xoff*xoff+yoff*yoff);
   ...
   for(j=outjbegin; j<=outjend; ++j)
   {
      ...
      for(i=outibegin; i<= outiend; ++i)
      {
         ...
         if(iin-inibegin < 0 || iin-inibegin >= innelements
         || jin-injbegin < 0 || jin-injbegin >= inlines)
