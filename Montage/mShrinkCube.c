...
{
   int       i, j, ii, jj, c, status, bufrow, fixedSize, split, mfactor;
   ...
   if (argc - optind < 3) 
   {
      fprintf(fstatus, "[struct stat=\"ERROR\", msg=\"Usage: %s [-f(ixed-size)] [-d level] [-h hdu] [-m factor] [-s statusfile] in.fits out.fits factor\"]\n", argv[0]);
      exit(1);
   }
