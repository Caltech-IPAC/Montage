...
int main(int argc, char **argv)
{
   ...
   if(argc < 3)
   {
      printf("[struct stat=\"ERROR\", msg=\"Usage: %s [-d level] [-r(eplace)] [-N(oWCS)] [-n noise_level] [-b bg1 bg2 bg3 bg4] [-t tbl col width epoch refmag] [-a array.txt] template.hdr out.fits (-t args can be repeated)\"]\n", argv[0]);
      exit(1);
   }
